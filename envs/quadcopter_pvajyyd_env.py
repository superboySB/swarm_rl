...
class QuadcopterPVAJYYdEnvCfg(DirectRLEnvCfg):
    ...
    # Scene
    scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=10000, env_spacing=20, replicate_physics=True)
...
class QuadcopterPVAJYYdEnv(DirectRLEnv):
    ...
    def _publish_debug_signals(self):
        ...
        w_w = state[10:13]
        w_b = quat_apply(quat_inv(self.robot.data.root_quat_w[env_id]), w_w)
        w = w_b.cpu().numpy()
